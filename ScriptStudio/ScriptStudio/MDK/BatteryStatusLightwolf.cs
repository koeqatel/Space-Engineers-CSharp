...
{ 
	...
	if	(Run_ThisScript)	{ 
		...
		/* 
		var display3 = (IMyTextPanel)GridTerminalSystem.GetBlockWithName("LCD info"); 
		display3.WritePublicText("", false); 
		*/ 
		//Pixel Tempelates 
		string P1 = ""; 
		string P2 = ""; 
		...
		var batterylist = new List<IMyTerminalBlock>();						//create new empty list 
		...
		{ 
			var batterylist_X_CustomName = batterylist[i].CustomName;							// Custom name of Battery Block 
			IMyBatteryBlock batterylist_X = GridTerminalSystem.GetBlockWithName(batterylist_X_CustomName) as IMyBatteryBlock; 
			...
			if (batterylist_X_MaxOutputValUnit == "W") { batterylist_X_MaxOutput_float_X = batterylist_X_MaxOutput_float_X / 1000;			// W to kW  
			} else if (batterylist_X_MaxOutputValUnit == "kW") { 																			// do nothing 
			...
			// Check W / kW / MW - Convert to kW 
			if (battery_X_MaxInputValUnit == "W") { battery_X_MaxInput_float_X = battery_X_MaxInput_float_X / 1000;				// W to kW  
			} else if (battery_X_MaxInputValUnit == "kW") { 																	// do nothing 
			...
			// Check W / kW / MW - Convert to kW 
			if (batt_X_MaxStoredValUnit == "Wh") { batt_X_MaxStored_float_X = batt_X_MaxStored_float_X / 1000;				// W to kW  
			...
			if (batterylist_X_Typ == "Batería")	{	batt_X_CurrInput = batterylist_X_Split[4].Split('l')[1].Trim(); 
			}	else 	{							batt_X_CurrInput = batterylist_X_Split[4].Split(':')[1].Trim();		// Current Input (Ex: 100 kW) 
			} 
			...
			// Check W / kW / MW - Convert to kW 
			if (batt_X_CurrInputValUnit == "W") { batt_X_CurrInput_float_X = batt_X_CurrInput_float_X / 1000;				// W to kW  
			} else if (batt_X_CurrInputValUnit == "kW") { 																			// do nothing 
			...
			// Check W / kW / MW - Convert to kW 
			if (batListX_CurrOutputValUnit == "W") { batListX_CurrOutput_float_X = batListX_CurrOutput_float_X / 1000;				// W to kW  
			...
			// Check W / kW / MW - Convert to kW 
			if (batX_CurrStoredValUnit == "Wh") { batt_X_CurrentStored_float_X = batt_X_CurrentStored_float_X / 1000;				// W to kW  
			...
			if (batterylist_X.Enabled) 	{ 
				...
				if (batterylist[i].IsFunctional)	{ 
					if (batterylist[i].IsWorking)	{ 
						...
						else if (batterylist_X.OnlyRecharge)	{	batt_OnlyRecharge	=	true;	} 
		...
		{ 
			...
			var allReactors_list = new List<IMyTerminalBlock>();					//create new empty list 
			...
			{ 
				var Reactor_X_CustomName = allReactors_list[i].CustomName;					// Custom name of Reactor 
				IMyReactor reactor_X = GridTerminalSystem.GetBlockWithName(Reactor_X_CustomName) as IMyReactor; 
				if (reactor_X.Enabled) { 
					if (allReactors_list[i].IsFunctional) { 
						...
						// Check W / kW / MW - Convert to kW 
						if (Reactor_X_MaxOutputValUnit == "W") { Reactor_X_MaxOutput_float_X = Reactor_X_MaxOutput_float_X / 1000;			// W to kW  
						} else if (Reactor_X_MaxOutputValUnit == "kW") { 																			// do nothing 
						...
						// Check W / kW / MW - Convert to kW 
						if (Reactor_X_CurrentOutputValUnit == "W") { Reactor_X_CurrentOutput_float_X = Reactor_X_CurrentOutput_float_X / 1000;			// W to kW  
						} else if (Reactor_X_CurrentOutputValUnit == "kW") { 																			// do nothing 
			...
			var allSolarPanels_list = new List<IMyTerminalBlock>();					//create new empty list 
			...
			{ 
				//var Solar_X_CustomName = allSolarPanels_list[i].CustomName;					// Custom name of Reactor 
				//IMySolarPanel Solar_X = GridTerminalSystem.GetBlockWithName(Solar_X_CustomName) as IMySolarPanel; 
					//if (allSolarPanels_list[i].IsFunctional) { 
						...
						// Check W / kW / MW - Convert to kW 
						if (Solar_X_MaxOutputValUnit == "W") 			{ Solar_X_MaxOutput_float_X = Solar_X_MaxOutput_float_X / 1000;		// W to kW  
						} else if (Solar_X_MaxOutputValUnit == "kW")	{ 																			// do nothing 
						...
						// Check W / kW / MW - Convert to kW 
						if (Solar_X_CurrentOutputValUnit == "W") { Solar_X_CurrentOutput_float_X = Solar_X_CurrentOutput_float_X / 1000;			// W to kW  
						} else if (Solar_X_CurrentOutputValUnit == "kW") { 																			// do nothing 
	...
	//	string myDataToString = batX_CurrStored_KW_All.ToString(); 
	//	int numVal = Int32.Parse(myDataToString); 
