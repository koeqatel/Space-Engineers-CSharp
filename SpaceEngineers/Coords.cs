...
namespace SpaceEngineers.UWBlockPrograms.BatteryMonitor
{
    public sealed class Program : MyGridProgram
    {
        ...
        public void Main(string args)
        {
            ...
            string sCoords = "(" + vCoords.X + ", " + vCoords.Y + ", " + vCoords.Z + ")";

            IMyCockpit oFlightSeat = GridTerminalSystem.GetBlockWithName("Cockpit: Flight Seat") as IMyCockpit;
            oFlightSeat.SetValue<Int64>("Content", 1);
            oFlightSeat.SetValue<Color>("FontColor", new Color(100, 100, 100));

            this.Log(oFlightSeat);

            oFlightSeat.SetValue<string>("Text", sCoords);
        }
